...
proptest! {
    #[test]
    fn test_submodule(initial in gen::commit::commit(), commits in collection::vec(gen::commit::commit(), 1..5)) {
        prop::test_submodule(initial, commits)
    }

    #[ignore = "segfault"]
    #[test]
    fn test_submodule_bare(initial in gen::commit::commit(), commits in collection::vec(gen::commit::commit(), 1..5)) {
